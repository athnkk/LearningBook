...
21.访问者：实列就是男女在不同状态表现得实列。表示一个作用于对象结构各个元素操作，它可以使得不改变元素的基础上重新定义新的操作。其实就是抽象一个类，里面定义访问ab两个方式，再具体出来。然后再定义一个类将所有访问者对象放在一个list进行访问。     
22.策略：定义算法封装起来。context传入不同算法同一个方法调用。     
23.备忘录：中间变量缓存状态。        
24.迭代器：提供类似itera迭代器方法类供外部使用。     
